//From https://github.com/VMormoris/GreenTea/blob/master/GreenTea/src/GreenTea/Core/uuid.h
#pragma once
...
std::ostream& operator<<(std::ostream& lhs, const uuid& rhs);

namespace std {
	template<>
	struct hash<uuid> {
		[[nodiscard]] size_t operator()(const uuid& id) const;
		/*
		{
